...
- 黑名单/标记功能现在支持UID，UID与用户名可以同时触发，但是以UID为优先
- 黑名单/标记的数据结构由字符串改为JSON，脚本升级的时候会自动转换数据结构
- 黑名单/标记升级后的数据结构缺少UID，当脚本识别到用户名的时候会自动写入UID，也可以手动在[名单管理]-[原始数据]，字段的UID可以手动填写（不建议不懂JSON语法的人改原始数据）
