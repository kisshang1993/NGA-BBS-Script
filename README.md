...
- 由于NGA查询属地/发帖记录接口添加了限流，查询过快会导致503错误，故此暂先改为手动点击查询
- 查询用户活动记录改为默认只查询一页回复记录
- 触发限流则需要等一段时间才可继续查询
...
- ⚡ **查看用户活动记录增强**
  ...
  功能进化，默认是查看用户3页的发帖与回复记录，不是特别准确，现在可以支持深度统计所有的发帖与回复，由于性能问题，此功能需要在查看用户活动面板之后手动点击`深度统计`进行手动检索，支持暂停/续查

    ![查看活动记录](https://s2.loli.net/2024/03/12/ZwbD8qJjYTrFokE.png)

- ⚡ **拉黑增强**

    如对现有黑名单功能不满足需要更强的屏蔽规则，可以安装此插件
    可以针对匿名用户、负声望、负威望、回帖数、注册天数进行屏蔽，支持联合校验校验

    [点击安装插件](https://greasyfork.org/zh-CN/scripts/489633-nga%E4%BC%98%E5%8C%96%E6%91%B8%E9%B1%BC%E4%BD%93%E9%AA%8C-%E6%8B%89%E9%BB%91%E5%A2%9E%E5%BC%BA)
...
![用户活动记录案例预览](https://s2.loli.net/2024/03/12/ZwbD8qJjYTrFokE.png)
...
| ---- | ------------ | ---- | ---- | ---- |
| 支持度 | 完全支持 | 部分支持 | 部分支持 | 注册菜单不支持 |
| 检查版本 | 4.12 | 2.13.0 | 4.11 | - |

> 由于脚本你管理器API的原因自4.4版本之后仅保持对Tampermonkey的兼容性检查，其他的脚本管理器不保证完全支持
